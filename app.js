...
const $=id=>document.getElementById(id),screens=['home','create','join','booth','result'];
let role=null,roomCode='',channel=null,pc=null,localStream=null,remoteStream=null,iceQueue=[],connected=false,shotIndex=0,shots=[],countdownTimer=null,lastStripBlob=null;
...
async function getCamera(){if(localStream)return localStream;if(!navigator.mediaDevices?.getUserMedia)throw Error('Camera API unavailable');localStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:1280},height:{ideal:720}},audio:false});$('localVideo').srcObject=localStream;return localStream}
function sendSignal(payload){if(channel)channel.send({type:'broadcast',event:'signal',payload})}
function createPeer(){
 if(pc)pc.close();pc=new RTCPeerConnection(rtcConfig);localStream?.getTracks().forEach(t=>pc.addTrack(t,localStream));
 ...
 pc.onconnectionstatechange=()=>{const s=pc.connectionState;if(s==='connected'){connected=true;$('captureBtn').disabled=false;$('boothStatus').textContent='Both cameras connected • Ready!';$('waitingText').textContent='Connected!';if(role==='host')show('booth')}else if(['failed','disconnected','closed'].includes(s)){connected=false;$('captureBtn').disabled=true;$('boothStatus').textContent='Connection interrupted…'}};
 return pc;
}
...
async function setupRoom(newRole,code){
 ...
 await channel.subscribe(async status=>{if(status==='SUBSCRIBED'){if(role==='host'){$('waitingText').textContent='Waiting for someone to join…'}else{$('boothStatus').textContent='Connecting to host…';show('booth')}sendSignal({type:'hello',role})}});
}
async function handleSignal(m){
 if(!m)return;
 if(m.type==='hello'&&role==='host'&&m.role==='guest'){createPeer();const offer=await pc.createOffer();await pc.setLocalDescription(offer);sendSignal({type:'offer',sdp:pc.localDescription});$('waitingText').textContent='Partner joined! Connecting…';return}
...
async function captureShot(){const w=620,h=760,canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d'),half=w/2;ctx.fillStyle='#111';ctx.fillRect(0,0,w,h);drawVideoCover(ctx,$('localVideo'),0,0,half,h);drawVideoCover(ctx,$('remoteVideo'),half,0,half,h);ctx.fillStyle='rgba(0,0,0,.42)';ctx.fillRect(0,h-58,w,58);ctx.fillStyle='#fff';ctx.font='700 16px DM Sans, sans-serif';ctx.textAlign='center';ctx.fillText('YOU',half/2,h-29);ctx.fillText('FRIEND',half+half/2,h-29);return canvas.toDataURL('image/jpeg',.62)}
function drawVideoCover(ctx,video,x,y,w,h){if(!video.videoWidth){ctx.fillStyle='#222';ctx.fillRect(x,y,w,h);return}const s=Math.max(w/video.videoWidth,h/video.videoHeight),sw=video.videoWidth*s,sh=video.videoHeight*s,dx=x+(w-sw)/2,dy=y+(h-sh)/2;ctx.save();ctx.beginPath();ctx.rect(x,y,w,h);ctx.clip();ctx.drawImage(video,dx,dy,sw,sh);ctx.restore()}
...
document.querySelectorAll('[data-back]').forEach(b=>b.onclick=()=>show(b.dataset.back));window.addEventListener('beforeunload',()=>{if(channel)sb.removeChannel(channel);if(pc)pc.close();localStream?.getTracks().forEach(t=>t.stop())});
